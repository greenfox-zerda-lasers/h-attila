...
app.get('/playlis-tracks/:playlist_id', function (req, res) {
  var playlists = tracks.filter (function (trackItem) {
    return trackItem.playlist_id.indexOf(parseInt(req.params.playlist_id)) >= 0;
  });
  res.json(playlists);
});
  ...
  tracks.forEach (function (trackItem, index) {
    if (trackItem.id === parseInt(req.params.track_id)) {
      if (trackItem.playlist_id.indexOf(req.params.track_id) < 0) {
        trackItem.playlist_id.push(parseInt(req.params.playlist_id));
      }
    }
  });
  res.json(tracks);
